...
//    @RequiresPermissions(value = "删除消息管理", authorities = "post:system:message:remove")
    ...
    public JsonResult queryListByStatus(String type){
        if("accept".equals(type)) {
            startPage();
            List<Message> list = messageService.queryListByStatus();
            ...
            startPage();
            List<Message> list = messageService.queryDelListByStatus();
            return JsonResult.ok().put("data",getDataTable(list));
        }
    }
