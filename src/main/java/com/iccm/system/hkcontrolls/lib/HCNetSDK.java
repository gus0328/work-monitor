...
public interface HCNetSDK extends StdCallLibrary {
    ...
    //    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(CommonUtil.getRealFilePath(HCNetSDK.class.getResource("/HKWS").getPath() + "/HCNetSDK").substring(1), HCNetSDK.class);
...
//网络(NET_DVR_NETCFG_V30结构)
    ...
    public static final int COMM_THERMOMETRY_ALARM = 0x5212;// 温度报警信息
    public static final int COMM_THERMOMETRY_DIFF_ALARM = 0x5211;// 温差报警上传
    public static final int COMM_FIREDETECTION_ALARM = 0x4991;// 火点检测报警信息
    public static final int COMM_UPLOAD_HEATMAP_RESULT = 0x4008;// 热度图报警上传 2014-03-21
...
//校时结构参数
    ...
    public static class NET_DVR_HEATMAP_RESULT extends Structure {// 热度图报警上传
        public int dwSize;
        ...
        public float wArrayLine;//图片像素点行值
        public float wArrayColumn;//图片像素点列值 （当行列值为0的时候，像素点值内存信息不存在）
        ...
        public float wDevInfoIvmsChannelEx;     //与NET_VCA_DEV_INFO里的byIvmsChannel含义相同，能表示更大的值。老客户端用byIvmsChannel能继续兼容，但是最大到255。新客户端版本请使用wDevInfoIvmsChannelEx。
        public byte byTimeDiffFlag;      /*时差字段是否有效  0-时差无效， 1-时差有效 */
        public char cStartTimeDifferenceH;      /*开始时间与UTC的时差（小时），-12 ... +14，+表示东区*/
        public char cStartTimeDifferenceM;      /*开始时间与UTC的时差（分钟），-30, 30, 45，+表示东区*/
        public char cStopTimeDifferenceH;        /*结束时间与UTC的时差（小时），-12 ... +14， +表示东区*/
        public char cStopTimeDifferenceM;       /*结束时间与UTC的时差（分钟），-30, 30, 45，+表示东区*/
    ...
    public static class NET_DVR_FIREDETECTION_ALARM extends Structure {// 火点检测报警
        ...
        public float wFireMaxTemperature;//火点最高温度[300℃~4000℃]
        public float wTargetDistance;//目标距离[100m ~ 10000m]
    ...
    public static class NET_DVR_THERMOMETRY_ALARM extends Structure {// 温度报警（检测温度和配置温度比较报警）
        public int dwSize;
        ...
        public String pPicBuff; /// 可见光图片指针
        public String pThermalPicBuff;// 热成像图片指针
        public String pThermalInfoBuff; // 热成像附加信息指针
    ...
    public static class NET_DVR_THERMOMETRY_DIFF_ALARM extends Structure {// 温差报警
        public int dwSize;
        public int dwChannel;//通道号
        ...
        public byte byAlarmRule;//0-大于，1-小于
        public byte byRuleCalibType;//规则标定类型 0-点，1-框，2线
        public NET_VCA_POINT[] struPoint = new NET_VCA_POINT[2];//点测温坐标（当规则标定类型为点的时候生效）数组下标0代表着AlarmID1，数组下标1代表着AlarmID2.
        public NET_VCA_POLYGON[] struRegion = new NET_VCA_POLYGON[2];//区域（当规则标定类型为框的时候生效）数组下标0代表着AlarmID1，数组下标1代表着AlarmID2.
...
/************************************多路解码器(end)***************************************/
    ...
    public static class NET_DVR_FINGER_PRINT_STATUS extends Structure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //指纹关联卡号
        public byte[] byCardReaderRecvStatus = new byte[MAX_CARD_READER_NUM_512]; //指纹读卡器状态
        ...
        public byte byRes1;
        public byte[] byErrorMsg = new byte[ERROR_MSG_LEN]; //下发错误信息，当byCardReaderRecvStatus为4时，表示已存在指纹对应的卡号
        public int dwCardReaderNo;
    ...
    public static class NET_DVR_FINGER_PRINT_INFO_COND extends Structure {
        ...
        public byte[] byEnableCardReader = new byte[MAX_CARD_READER_NUM_512];  //指纹的读卡器是否有效，数组下标表示读卡器序号，数组值：0- 无效，1- 有效
        public int dwFingerPrintNum;    //设置或获指纹数量，获取时置为0xffffffff表示获取所有指纹信息
        public byte byFingerPrintID;     //指纹编号，有效值范围为1~10，获取时置为0xff表示该卡所有指纹
        public byte byCallbackMode;     //设备回调方式：0- 已向所有读卡器下发完成，1- 在时间段内只下发了部分也返回
    ...
    public static interface FDrawFun extends StdCallCallback {
        public void invoke(NativeLong lRealHandle, WinDef.HDC hDc, int dwUser);
    }
    ...
    NativeLong NET_DVR_PlayBackByTime(NativeLong lUserID, NativeLong lChannel, NET_DVR_TIME lpStartTime, NET_DVR_TIME lpStopTime, WinDef.HWND hWnd);
...
interface PlayCtrl extends StdCallLibrary {
    PlayCtrl INSTANCE = (PlayCtrl) Native.loadLibrary("PlayCtrl",
            PlayCtrl.class);
//    PlayCtrl INSTANCE = (PlayCtrl) Native.loadLibrary(CommonUtil.getRealFilePath(HCNetSDK.class.getResource("/HKWS").getPath() + "/PlayCtrl.dll").substring(1),
//            PlayCtrl.class);
...
interface GDI32 extends WinDef {
    ...
    WinNT.HANDLE CreateSolidBrush(int icolor);
}

//windows user32接口,user32.dll in system32 folder, 在设置遮挡区域,移动侦测区域等情况下使用
interface USER32 extends WinDef {
    ...
    int FillRect(HDC hDC, RECT lprc, WinNT.HANDLE hbr);
}
