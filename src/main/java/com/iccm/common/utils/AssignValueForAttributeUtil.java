...
/**
* @ClassName AssignValueForAttributeUtil
* @Description TODO(通过反射给javabean属性赋值)
* @author GXJ
* @Date 2017-4-1 下午12:15:18
* @version 1.0.0
*/
       ...
       for (Method method : methods) {
           ...
           if(method.getName().equals(method_name))
           {
               ...
               try {
                   ...
                   {
                       method.invoke(obj,((Integer)Integer.parseInt((value+"").trim())).intValue());
                       break;
                   }else if(parameterC[0] == float.class|| parameterC[0] == Float.class){
                       method.invoke(obj, ((Float)Float.parseFloat((value+"").trim())).floatValue());
                       break;
                   }else if(parameterC[0] == double.class || parameterC[0] == Double.class)
                   {
                       method.invoke(obj, ((Double)Double.parseDouble((value+"").trim())).doubleValue());
                       break;
                   }else if(parameterC[0] == byte.class)
                   {
                       method.invoke(obj, ((Byte)Byte.parseByte((value+"").trim())).byteValue());
                       break;
                   ...
                   {
                       method.invoke(obj, ((Boolean)Boolean.parseBoolean((value+"").trim())).booleanValue());
                       break;
                   }else
                   {
                       method.invoke(obj,parameterC[0].cast((value+"").trim()));
                       break;
                   }
